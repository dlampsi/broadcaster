...
func Test_ForPrint(t *testing.T) {
	...
	raw := Print("\n")
	require.Equal(t, "a0feed 0.0.0", raw)

	CommitHash = "fakeone"
	withCommit := Print("\n")
	require.Equal(t, "a0feed 0.0.0\nCommit: fakeone", withCommit)
	...
	Release = "5.5.6"
	withRelease := Print("\n")
	require.Equal(t, "a0feed 5.5.6\nCommit: fakeone\nBuild time: 2222-02-18_08:32:16", withRelease)

	diffSep := Print("; ")
	require.Equal(t, "a0feed 5.5.6; Commit: fakeone; Build time: 2222-02-18_08:32:16", diffSep)
}
